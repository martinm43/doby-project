...
for season_year_start in range(start_year, end_year, -1):
    ...
    df = df.rename(
        columns={"R": "HomeTeamRuns", "RA": "AwayTeamRuns", "Unnamed: 2": "dummy"}
    )
...
Games.update(away_team_runs=0).where(
    Games.home_team_runs == "Game Preview, and Matchups"
).execute()
Games.update(home_team_runs=0).where(
    Games.home_team_runs == "Game Preview, and Matchups"
).execute()
